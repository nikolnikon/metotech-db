...
XML;
	...
	public function insertItem($arFieldVals) {
		...
		$query = "SELECT `id` FROM `wwwmetotechru_metalls`.`special_prices` WHERE `price_name` = '".mysql_real_escape_string($this->_priceName)."'";
		try {
	...
	private function _load($conditions) {
		...
		if (! is_null($conditions) && is_array($conditions)) {
			...
			$gprice_query = "SELECT `id` FROM `wwwmetotechru_metalls`.`general_price` WHERE ";
		...
		// получаем все необходимые записи общего прайс-листа из таблицы general_price
		$query_1 = "SELECT `id` FROM `wwwmetotechru_metalls`.`special_prices` WHERE `price_name` = '".mysql_real_escape_string($this->_priceName)."'";
		$query_2 = "SELECT `gprice_id` FROM `wwwmetotechru_metalls`.`prices_mapping` WHERE `sprice_id` = ($query_1) $gprice_query";
	...
	private function _getFilterQuery($table, $fields, $returned_fields) {
		...
		$query .= " FROM `wwwmetotechru_metalls`.`$table` WHERE ";
