...
function getFilterQuery($table, $returned_fields, $conds=null, $order_field=null) {
	...
	$query .= " FROM `metotech_metalls_strip`.`$table`";
	if (! is_null($conds)) {
		$query .= " WHERE ";
	...
	try {
		...
		foreach ($options as $option) {
			if ($param == "material") {
				...
				$query = "SELECT DISTINCT `temp_solid` FROM `metotech_metalls_strip`.`heater_surface_power` WHERE `temp_solid` < ".mysql_real_escape_string($max_heater_temp);
				$temps = $db->select($query);
				unset($arr);
				foreach ($temps as $temp) {
					$arr[] = $temp['temp_solid'];
